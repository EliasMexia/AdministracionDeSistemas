...
function Liberar-Puertos-Web {
    ...
    # EL ANIQUILADOR NUCLEAR: Matamos los procesos fantasma a nivel de sistema operativo
    ...
    Write-Host "Entorno 100% liberado y limpio (Modo Cache Local Activado)." -ForegroundColor Green
}
function Instalar-Apache {
    ...
    if ($origen -eq "1") {
        ...
        # AQUI OCURRIO LA MAGIA: Le avisamos a la funcion que busque "Apache"
...
"@
    ...
    # --- INICIO DE SECCIÓN: GENERACIÓN DE INDEX PERSONALIZADO ---
...
function Instalar-Nginx {
    ...
    if ($origen -eq "1") {
        ...
        Write-Host "Extrayendo Nginx (Modo Ultra Silencioso, sin letras rojas)..."
        ...
        # AQUI OCURRIO LA MAGIA: Le avisamos a la funcion que busque "Nginx"
...
"@
    ...
    # --- INICIO DE SECCIÓN: GENERACIÓN DE INDEX SIN CARACTERES ESPECIALES ---
    Write-Host "Generando Monitor de Estado en index.html..." -ForegroundColor Yellow
...
"@
    # Asegurar que la carpeta html exista y crear el archivo
...
function Instalar-IIS-Web {
    ...
    # BARRIDO NUCLEAR DE SITIOS: Borramos cualquier sitio que exista para evitar secuestros de puertos
    ...
    if ($isSSL) {
        ...
        Write-Host "Reiniciando IIS de forma agresiva para aplicar modulos..."
        ...
        # EL PARCHE DE ORO PARA EL SSL: Forzamos el enlace directo al proveedor de IIS
...
function Instalar-IIS-FTP {
    ...
    # EL PARCHE ANTI-530: Desactivamos el User Isolation global de la Practica 5 para este sitio
    Set-ItemProperty "IIS:\Sites\FTP_Practica7" -Name ftpServer.userIsolation.mode -Value 0
    
    # Limpiamos reglas de autorizacion heredadas que puedan bloquear
...
function Navegar-Descargar-FTP {
    ...
    # Ajuste de ruta según tu captura: repositorio/Servicio
    ...
    # Filtramos archivos .zip (quitamos espacios y saltos de linea vacios)
