...
configurar_scope() {
    log_info "--- CONFIGURACIÓN DEL ÁMBITO DHCP ---"
    verificar_ip_fija

    echo "Interfaces disponibles:"
    ip link show | grep "enp" | awk -F: '{print $2}' | tr -d ' '
    read -p "Nombre del Adaptador para DHCP (ej. enp0s8): " INT_IFACE
    if [ -z "$INT_IFACE" ]; then log_error "Interfaz vacía."; return; fi
...
EOF
    ...
    if systemctl is-active --quiet isc-dhcp-server; then
        log_ok "DHCP configurado y corriendo exitosamente."
    else
        log_error "Error iniciando servicio DHCP."
    fi
    read -p "Enter para continuar..."
}
